...
class CuentaAhorro(Cuenta):
    def retirar(self, monto):
        if monto > 1000:
            print("En CuentaAhorro, no se puede retirar más de 1000 en una sola transacción.")
        elif monto <= self.saldo:
            self.saldo -= monto
            print(f"Retiro de {monto} realizado en CuentaAhorro. Nuevo saldo: {self.saldo}")
        else:
            print("Saldo insuficiente")
...
class CuentaCorriente(Cuenta):
    def retirar(self, monto):
        ...
        if monto <= self.saldo + limite_sobregiro:
            self.saldo -= monto
            print(f"Retiro de {monto} realizado en CuentaCorriente. Nuevo saldo: {self.saldo}")
        else:
            print("Sobregiro máximo excedido en CuentaCorriente.")
...
class CuentaEmpresa(Cuenta):
    def retirar(self, monto):
        ...
        if total <= self.saldo:
            self.saldo -= total
            print(f"Retiro de {monto} realizado en CuentaEmpresa con una comisión de {comision}. Nuevo saldo: {self.saldo}")
        else:
            print("Saldo insuficiente para cubrir el monto y la comisión.")
